...
class YoloPredictor(BasePredictor, QObject):
    yolo2main_trail_img = pyqtSignal(np.ndarray)  # 轨迹图像信号
    yolo2main_box_img = pyqtSignal(np.ndarray)  # 绘制了标签与锚框的图像的信号
    yolo2main_status_msg = pyqtSignal(str)  # 检测/暂停/停止/测试完成等信号
    yolo2main_fps = pyqtSignal(str)  # fps

    yolo2main_labels = pyqtSignal(dict)  # 检测到的目标结果（每个类别的数量）
    yolo2main_progress = pyqtSignal(int)  # 进度条
    yolo2main_class_num = pyqtSignal(int)  # 当前帧类别数
    yolo2main_target_num = pyqtSignal(int)  # 当前帧目标数
